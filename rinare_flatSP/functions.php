...
/* スマフォ用のテーマは、posts_per_pageを５にしたい */
function change_posts_per_page($query) {
    if ( is_admin() || ! $query->is_main_query() )
        return;
 
    if ( $query->is_archive() || $query->is_front_page() || $query->is_home()) {
...
function rinare_comment($comment, $args, $depth) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<div id="comment-<?php comment_ID(); ?>" class="clearfix">
			...
			<!-- アバター -->	
			<div class="authorImg">
				<?php echo get_avatar($comment,$size='80',$default=get_template_directory_uri().'/images/guest'.(mt_rand(1,5)).'.png' ); ?>
			</div>
...
function pagination($_all_page_num = '', $_show_item_range = 2) {
    ...
    if ($_all_page_num > 1) {
        // Page X of Y の表示
        echo "<div class=\"pages\">".$paged."/".$_all_page_num." pages</div><div class=\"pagination\">";
...
function gAd_top() {
    return '<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
			...
			<ins class="adsbygoogle"
			     ...
			     data-ad-slot="8409607579"></ins>
			<script>
			(adsbygoogle = window.adsbygoogle || []).push({});
			</script>'; 
}
add_shortcode('gAd_300top', 'gAd_top');
...
function gAd_entry() {
    return '<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
			...
			<ins class="adsbygoogle"
			     style="display:inline-block;width:300px;height:250px"
			     data-ad-client="ca-pub-7903067641116583"
			     data-ad-slot="4727358378"></ins>
			<script>
			(adsbygoogle = window.adsbygoogle || []).push({});
			</script>'; 
}
add_shortcode('gAd_300entry', 'gAd_entry');


/* タイトル下とあるが、記事の関連と記事フッターの間に小さい広告を */
function gAd_tail() {
    return '<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
			<!-- spタイトル下 -->
			<ins class="adsbygoogle"
			     ...
			     data-ad-client="ca-pub-7903067641116583"
			     data-ad-slot="7736781971"></ins>
			<script>
			(adsbygoogle = window.adsbygoogle || []).push({});
			</script>'; 
}
add_shortcode('gAd_320x100', 'gAd_tail');
